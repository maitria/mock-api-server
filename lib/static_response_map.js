...
void function () {
  var buildResponseMap, buildStaticResponseEntry, cache$, each, entryAllowedForRequest, extractMethod, filter, path, patternMatcher, size, sortBy, stripExtension, url;
  path = require('path');
  ...
  extractMethod = function (filename) {
    var _, cache$1, method;
    cache$1 = filename.split(path.sep);
    _ = cache$1[0];
    method = cache$1[1];
  ...
  buildStaticResponseEntry = function (filename, content) {
    var cache$1, cache$2, method, pathname, query;
