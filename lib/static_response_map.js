...
void function () {
  var buildResponseMap, buildStaticResponseEntry, cache$, each, entryAllowedForRequest, extractQueryFromFilename, filter, last, matchesPattern, size, sortBy, stripExtension;
  ...
  entryAllowedForRequest = function (request, responseMapEntry) {
    ...
    each(responseMapEntry.query, function (value, name) {
      if (!matchesPattern(request.query[name], value))
